...
import { LecNumState, Interval, IntervalsEachDays, GradRecord } from './interfaces';
...
*/
const sql_recommend = (intervals : IntervalsEachDays) : string => { 
    let sql = `
    SELECT 
        ...
        FROM
            ...
            for(day in intervals) {
                for(time of intervals[day]) {
                    sql += `                    `;
                    sql += `OR ("요일" = ${db.escapeLiteral(day)} AND "시작시간" >= ${db.escapeLiteral(time.start as string)}::time AND "끝시간" <= ${db.escapeLiteral(time.end as string)}::time)\n`
                }
            }
...
const sql_details = (lec_num : number) : string[] => {
    ...
    FROM
        ...
        FROM
            ...
            FROM ${tables.lec_info} AS info
            JOIN ${tables.prev_lec_infos} AS prev_info
            ON info."수업번호" = ${db.escapeLiteral(lec_num.toString())} 
                AND info."학수번호" = prev_info."학수번호"
                AND info."설강소속코드" = prev_info."설강소속코드"
                AND info."대표교강사명" = prev_info."대표교강사명") AS prev_lecs
        JOIN ${tables.pn_each_depart} AS depart
        ON depart."수업번호" = prev_lecs."수업번호"
        GROUP BY prev_lecs."수업번호") AS dep_prev_lecs
    JOIN ${tables.prev_pn_info} AS pn
    ON pn."수업번호" = dep_prev_lecs."수업번호"
    ORDER BY pn."수업번호";
            `,
